...
def painel():
    ...
    # ... (CÓDIGO FINANCEIRO E METAS EXISTENTE - MANTENHA IGUAL) ...
    # (Vou resumir a parte de cima para focar no novo código, mas você deve MANTER tudo que já estava lá)

    # --- 1. ENTRADAS & PREVISÃO ---
    recebido_mes = db.session.query(func.sum(Pagamento.valor)).filter(extract('month', Pagamento.data_pagamento) == mes_atual, extract('year', Pagamento.data_pagamento) == ano_atual).scalar() or 0
    ...
    vendas_hoje = db.session.query(func.sum(Venda.valor_final)).filter(func.date(Venda.criado_em) == hoje, Venda.status != 'cancelada', Venda.status != 'orcamento').scalar() or 0
    atingimento_dia_perc = (float(vendas_hoje) / meta_diaria_alvo) * 100 if meta_diaria_alvo > 0 else 0

    # --- 3. A PAGAR ---
    a_pagar_mes = db.session.query(func.sum(Despesa.valor)).filter(extract('month', Despesa.data_vencimento) == mes_atual, extract('year', Despesa.data_vencimento) == ano_atual, Despesa.status == 'pendente').scalar() or 0
    ...
    # =================================================================
    # --- 5. INDICADORES OPERACIONAIS (NOVO) ---
    # =================================================================
    
    # 1. Fila (Pendente)
    # Consideramos itens que não foram iniciados (pendente) de vendas não canceladas
    ...
    op_execucao = db.session.query(ItemVenda).join(Venda).filter(
        ItemVenda.status == 'em_producao',
        Venda.status != 'cancelada'
    ).all()

    # 3. Prontos (Concluídos mas não entregues)
    ...
    # Filtramos itens entregues cujas vendas foram criadas (ou atualizadas) neste mês
    # (Idealmente teríamos data_entrega no item, mas usamos a data da venda como proxy ou data atual)
    op_finalizados = db.session.query(ItemVenda).join(Venda).filter(
        ItemVenda.status == 'entregue',
        extract('month', Venda.criado_em) == mes_atual, # Ou updated_at se tiver
    ...
    for i in range(5, -1, -1):
        ...
        soma_v = db.session.query(func.sum(Venda.valor_final)).filter(extract('month', Venda.criado_em) == mes_iter, extract('year', Venda.criado_em) == ano_iter, Venda.status != 'cancelada', Venda.status != 'orcamento').scalar() or 0
        chart_data_vendas.append(float(soma_v))

        soma_d = db.session.query(func.sum(Despesa.valor)).filter(extract('month', Despesa.data_competencia) == mes_iter, extract('year', Despesa.data_competencia) == ano_iter).scalar() or 0
        chart_data_despesas.append(float(soma_d))

    cats_db = db.session.query(Despesa.categoria, func.sum(Despesa.valor)).filter(extract('month', Despesa.data_competencia) == mes_atual, extract('year', Despesa.data_competencia) == ano_atual).group_by(Despesa.categoria).all()
    doughnut_labels = [c[0].title() for c in cats_db]
    doughnut_data = [float(c[1]) for c in cats_db]
    
    if not doughnut_data:
        status_db = db.session.query(Venda.status, func.count(Venda.id)).filter(extract('month', Venda.criado_em) == mes_atual).group_by(Venda.status).all()
        doughnut_labels = [s[0].upper() for s in status_db]
        doughnut_data = [int(s[1]) for s in status_db]

    faturamento_mes = db.session.query(func.sum(Venda.valor_final)).filter(extract('month', Venda.criado_em) == mes_atual, extract('year', Venda.criado_em) == ano_atual, Venda.status != 'cancelada', Venda.status != 'orcamento').scalar() or 0
